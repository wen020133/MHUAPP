...
-(void)setListModel:(WJOrderGoodListModel *)listModel
{
    ...
    [_contentImg sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",_listModel.img]] placeholderImage:[UIImage imageNamed:@"default_nomore.png"] completed:nil];
    NSString *price =@"";
    ...
    _title.frame = CGRectMake(TAG_Height+DCMargin, 5, kMSScreenWith- DCMargin * 4-TAG_Height-width, 40);

    NSString *saleCount = [NSString stringWithFormat:@"%@",_listModel.goods_attr];
    _type.text  = saleCount;
    _type.frame = CGRectMake(TAG_Height+DCMargin, _title.Bottom+5, _title.width, 20);
...
-(void)setItem:(WJOrderWaitPingjiaAndSuccessItem *)item
{
    if (item!=_item) {
        _item = item;
    }
    [_contentImg sd_setImageWithURL:[NSURL URLWithString:_item.img] placeholderImage:[UIImage imageNamed:@"default_nomore.png"] completed:nil];
    NSString *price =@"";
    ...
    _title.text = _item.goods_name;
    _title.frame = CGRectMake(TAG_Height+DCMargin, 5, kMSScreenWith- DCMargin * 4-TAG_Height-width, 40);

    NSString *saleCount = [NSString stringWithFormat:@"%@",_item.goods_attr];
    _type.text  = saleCount;
    _type.frame = CGRectMake(TAG_Height+DCMargin, _title.Bottom+5, _title.width, 20);

    _Num.frame =CGRectMake(kMSScreenWith-width-10, _oldprice.Bottom+5, width, 20);
    _Num.text =  [NSString stringWithFormat:@"x%@",_item.goods_number];
}

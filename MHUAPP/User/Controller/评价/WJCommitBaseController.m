...
@interface WJCommitBaseController ()<UICollectionViewDelegate,UICollectionViewDataSource,JJPhotoDelegate,DWQImagePickerSheetDelegate>{
    NSString *pushImageName;
    //添加图片提示
    UILabel *addImageStrLabel;
}
@property (nonatomic, strong) WJImagePickerSheet *imgPickerActionSheet;
@end

@implementation WJCommitBaseController
...
- (void)addNewImg{
    if (!_imgPickerActionSheet) {
        _imgPickerActionSheet = [[WJImagePickerSheet alloc] init];
...
- (void)changeCollectionViewHeight{
    ...
    if (_collectionFrameY) {
        ...
        _pickerCollectionView.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, (((float)[UIScreen mainScreen].bounds.size.width-64.0) /4.0 +20.0)* ((int)(_arrSelected.count)/4 +1)+10.0);
    }
